...
- (void)incrementValueBy:(NSInteger)delta {
    if (!_value) {
#warning TODO: is throwing the right thing?
        @throw RLMException(@"Cannot increment a RLMNullableInteger property whose value is nil");
    }
...
- (void)incrementValueBy:(NSInteger)delta {
    ...
    if (table->is_null(_colIndex, _row.get_index())) {
#warning TODO: is throwing the right thing?
        @throw RLMException(@"Cannot increment a RLMInteger property whose value is nil");
    }
